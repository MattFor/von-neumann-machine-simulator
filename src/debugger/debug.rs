...
impl Debug {
    ...
    pub fn send(&mut self, state: &mut AppState, raw: String) {
        ...
        if(raw_split.is_empty()) { return; }

        let command = raw_split[0];

        match command {
            "SetMem" => {
                let valid = self.setMemory(state, raw_split);
                self.command_history.push((raw.clone(), valid));
            },
            "SetAcc" => {
                let valid = self.setAccumulator(state, raw_split);
                self.command_history.push((raw.clone(), valid));
            },
            "Help" => {
            ...
            _ => { self.command_history.push((raw.clone(), false)); }
        }
    }

    fn setMemory(&self, state: &mut AppState, arguments: Vec<&str>) -> bool {
        ...
        if(address >= 255 || address < 0) { return false; }
    ...
    fn setAccumulator(&self, state: &mut AppState, arguments: Vec<&str>) -> bool {
    ...
    pub fn help(&mut self) {
        self.command_history.push(("WARNING! Debug mode is unsafe at this point!".to_string(), false));
        self.command_history.push(("Use only when you know what you're doing".to_string(), false));
        self.command_history.push((" ".to_string(), false));
        self.command_history.push(("Available Commands:".to_string(), true));
        self.command_history.push(("SetMem <ADDRESS [0-255]> <VALUE DEC/HEX>".to_string(), true));
        self.command_history.push(("SetAcc <VALUE DEC>".to_string(), true));
